...
"""
def taunuc_3(Txi,rhox,time):
    T = np.power(10.0,Txi)
    rho = np.power(10.0, rhox)
    return (rho/(12.0*mu))/eCC.Reaction(rho,T,6.0,13.7,0.5)-time

def taunuc_2(rhox,Txi,time):
    T = np.power(10.0,Txi)
    rho = np.power(10.0, rhox)
    return (rho/(12.0*mu))/eCC.Reaction(rho,T,6.0,13.7,0.5)-time
"""

Tx = np.linspace( 7.0, 9.0, 20)
rhoCC=[]
tau1=[]
dat_print=[]
#dat_print_2=[]
...
for xx in Tx:
    ...
    rhoCC.append(Tc)
    dat_print.append([Tc,xx])
    #Tc = brentq(taunuc , 2.0 , 15.0 ,args=(xx,1.0))
    #tau1.append(Tc)
   # dat_print_2.append([Tc,xx])
    # print nunu.QnuTotal(np.power(10.0,Tc),np.power(10.0,xx),12.0,6.0), eCC.QCC(np.power(10.0,Tc),np.power(10.0,xx),6.0,12.0,0.5)
    i+=1

#np.savetxt('T_ignition.dat', dat_print,fmt='%1.7e')
#exit()
...
def Torque_dip(t,y,ffx):
    ...
    tau = ffx[2]
    #average atomic weight and charge
    ration=0.0; XmassT=0.0;
    for i in range(len(aion)):
        ration+=xmass[i]/aion[i]
        XmassT+=xmass[i]*zion[i]/aion[i]
    abar = 1.0e0/ration
    zbar   = abar * XmassT
   ...
   # Omegax = Omega_j(y[0])
    ...
    cvv = cv_gas( np.power(10.0,rhowdx), Tdim*y[1], aion, xmass,zion)
    #print cvv
    ec2 = y[1] * drhodj * np.log(10.0) * ec1 / cvv + np.log(10.0) * np.power(10.0,t) * tau * abar * mu * (-nunu.QnuTotal(np.power(10.0,rhowdx),Tdim * y[1],abar,zbar) + eCC.QCC(np.power(10.0,rhowdx),Tdim*y[1],6.0,12.0,xmass[0]))/(np.power(10.0,rhowdx) * kappa * Tdim * cvv)
...
def Terms_eqs(t,y,ffx):
   ...
   # Omegax = Omega_j(y[0])
    ...
    term2 = np.log(10.0) * np.power(10.0,t) * tau * abar * mu * (  eCC.QCC(np.power(10.0,rhowdx),Tdim*y[1],zbar,abar,xmass[0])-nunu.QnuTotal(np.power(10.0,rhowdx),Tdim * y[1],abar,zbar)) / ( np.power(10.0,rhowdx) * kappa * y[1] * Tdim * cvv )
...
xnamefile = 'TevolutionWD_energyKuantay'
def Tevolution(jwd0,rhowd0,T0,cc):
    if cc==0:
        xname = '_'.join([xnamefile,str(int(np.log10(T0)*10.0)),str(int(args.mass*100.0))]) 
    else:
        xname = '_'.join([xnamefile,'fluxconst',str(int(np.log10(T0)*10.0)),str(int(args.mass*100.0))]) 
    ...
    incr2 = 0
    dt =1e-6
    print rho_x, Interp1D_ld(np.log10(yy[1]*Tdim),Tx,rhoCC)
    tnow = t0
    while rho_x<rhobeta and rho_x<2.4*Interp1D_ld(np.log10(yy[1]*Tdim),Tx,rhoCC):
        ...
        #print Omegaxnew 
            ...
            np.savetxt(FN,[[ np.power(10.0,(tnew))*tauD , rho_x ,yy[0] , Omegawd_0 * Omegaxnew , yy[1], Reqnew * Reqwd_0,termss[0]*delta_time,termss[1]*delta_time]],fmt='%1.7e')
            i=i+1
            tnow = tnew
...
#r0 = drr*1e-2
    ...
    while Temperature.successful() and  rho_x<rhobeta and rho_x<Interp1D_ld(np.log10(Temperature.y[1]*Tdim),Tx,rhoCC):
       ...
       # print termss ,  eCC.QCC(np.power(10.0,rho_x),Tdim*Temperature.y[1],6.8,13.7,xmass[0]), nunu.QnuTotal(np.power(10.0,rho_x),Tdim * Temperature.y[1],13.7,6.8) 
        ...
        #np.savetxt(FN,[[ np.power(10.0,(Temperature.t))*tauD , rho_x ,Temperature.y[0] , Omegawd_0 * Omegaxnew , Temperature.y[1], Reqnew * Reqwd_0,termss[0]*delta_time,termss[1]*delta_time]],fmt='%1.7e')
    #FN.close()
    print rho_x, Interp1D_ld(np.log10(Temperature.y[1]*Tdim),Tx,rhoCC)
    return evol_1[-1][0]


x_B=np.linspace(6,10,10)
timescale= []
FN = open('timesscales_Energy_146_T8.dat','w')
for xxB in x_B:
    ...
    Evol1 = Tevolution_2( [ jwd_0, 1e8/Tdim ] , 5e-3, rhowd_0, Bs0 , 0.0 )
    Evol1_flux = Tevolution_2( [ jwd_0 , 1e8/Tdim ] , 5e-3 , rhowd_0 , Bs0 , 1.0)
...
Evol1 = Tevolution_2([jwd_0,1e8/Tdim],5e-3,rhowd_0,1e6,0)
#Evol1=Tevolution(jwd_0,rhowd_0,1e8,0)
print Evol1/(1e6*yr)
exit()

Evol1_flux = Tevolution_2([jwd_0,1e8/Tdim],1e-2,rhowd_0,1.0)
#Evol1_flux = Tevolution(jwd_0,rhowd_0,1e8,1)

plt.plot([Evol1[j][0] for j in range(len(Evol1)-1)],[ Evol1[j][3]*c/Rdim for j in range(len(Evol1)-1)],'ro')
plt.plot([Evol1_flux[j][0] for j in range(len(Evol1_flux)-1)],[ Evol1_flux[j][3]*c/Rdim for j in range(len(Evol1_flux)-1)],'bo')
plt.show()



plt.semilogy([ ( Evol1[j][1] ) for j in range(len(Evol1)-1)],[ Tdim*Evol1[j][4] for j in range(len(Evol1)-1)],'k-')
plt.semilogy([ ( Evol1_flux[j][1] ) for j in range(len(Evol1_flux))],[ Tdim*Evol1_flux[j][4] for j in range(len(Evol1_flux))],'k--')
plt.semilogy(rhoCC,np.power(10.0,Tx),'b-')
plt.show()
"""

Evol2 =  Tevolution_2([jwd_0,1e7/Tdim],1e-3,rhowd_0,0)
Evol2_flux =  Tevolution_2([jwd_0,1e7/Tdim],1e-3,rhowd_0,1.0)
#Evol2=Tevolution(jwd_0,rhowd_0,1e7,0)
#Evol2_flux = Tevolution(jwd_0,rhowd_0,1e7,1)
...
Evol3 = Tevolution_2([jwd_0,1e6/Tdim],1e-3,rhowd_0,0)
#Evol3=Tevolution(jwd_0,rhowd_0,1e6,0)
#Evol3_flux = Tevolution(jwd_0,rhowd_0,1e6,1)
Evol3_flux =  Tevolution_2([jwd_0,1e6/Tdim],1e-3,rhowd_0,1.0)

plt.semilogy([ Evol2[j][1]  for j in range(len(Evol2)-1)],[ Tdim*Evol2[j][4] for j in range(len(Evol2)-1)],'b-')
plt.semilogy([ Evol2_flux[j][1]  for j in range(len(Evol2_flux)-1)],[ Tdim*Evol2_flux[j][4] for j in range(len(Evol2_flux)-1)],'b--')

plt.semilogy([ Evol3[j][1]  for j in range(len(Evol3)-1)],[ Tdim*Evol3[j][4] for j in range(len(Evol3)-1)],'r-')
plt.semilogy([ Evol3_flux[j][1]  for j in range(len(Evol3_flux)-1)],[ Tdim*Evol3_flux[j][4] for j in range(len(Evol3_flux)-1)],'r--')
...
#print evol_1[-2][1],  (np.power(10,evol_1[-2][0]))*tauD/(1e6*yr)
"""
